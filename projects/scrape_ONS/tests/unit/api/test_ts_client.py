...
class TestTSApiClient:
    ...
    def test_get_dataset_url(self, mock_get):
        ...
        # The URL should include the dataset ID, edition, version and proper area format
        expected_url = "https://api.beta.ons.gov.uk/v1/datasets/TS030/editions/2021/versions/1/json?area-type=ctry,E92000001"
    ...
    def test_batch_get_dataset_data(self, mock_get):
        ...
        mock_response.json.return_value = {
            "dimensions": [
                ...
                {
                    "name": "religion_tb",
                    "options": [{"id": "1", "name": "No religion"}]
                }
            ],
            "observations": {"E92000001,1": "20715664"}
        }
        mock_get.return_value = mock_response
